...
def get_foot_pose_traj_msg(time_traj, foot_idx_traj, foot_traj, torso_traj):
    ...
    for i in range(num):
        ...
        foot_h=0.1#抬腿高度
        taijie=0.13+0.125+0.05

        x_f=foot_traj[i][0]-taijie*math.cos(torso_yaw)
        y_f=foot_traj[i][1]-taijie*math.sin(torso_yaw)
        ...
        if x_f<=0:
            ...
            if i == 0:
                foot_pose_msg.torsoPose[2] = torso_traj[i][2]- 0.2*foot_now_h   # 设置躯干姿态
                foot_pose_msg.torsoPose[3] = torso_traj[i][3]
                ...
                for xx in range(7):
                    ...
                    if xx<=3:
                        ...
                        step_fp.footPose[0] =  x_f * (xx / 3) 
                        step_fp.footPose[1] =  y_f * (xx / 3)
                        # print((np.pi/2) * xx / 3)
                    else:
                        step_fp.footPose[2] = foot_h * np.sin(np.pi/2+(np.pi/2) * (xx-3) / 3)+foot_traj[i][2]  -foot_now_h
                        step_fp.footPose[0] =  (foot_traj[i][0]-x_f) * ((xx-3) / 3) + x_f
                        step_fp.footPose[1] =  (foot_traj[i][1]-y_f) * ((xx-3) / 3) + y_f
                        # print(np.pi/2+(np.pi/2) * (xx-3) / 3)
            ...
            if i == 1:
                foot_pose_msg.torsoPose[2] = torso_traj[i][2]- foot_now_h   # 设置躯干姿态
                foot_pose_msg.torsoPose[3] = torso_traj[i][3]
                ...
                for xx in range(7):
                    ...
                    if xx<=3:
                        step_fp.footPose[2] = z_f * np.sin((np.pi/2) * xx / 3)  
                        step_fp.footPose[0] =  0.8 * x_f * (xx / 3) 
                        step_fp.footPose[1] =  0.8 * y_f * (xx / 3)
                        # print((np.pi/2) * xx / 3)
                    else:
                        step_fp.footPose[2] = foot_h * np.sin(np.pi/2+(np.pi/2) * (xx-3) / 3)+foot_traj[i][2]  -foot_now_h
                        step_fp.footPose[0] =  (foot_traj[i][0]-x_f) * ((xx-3) / 3) + x_f*0.8
                        step_fp.footPose[1] =  (foot_traj[i][1]-y_f) * ((xx-3) / 3) + y_f*0.8
                        # print(np.pi/2+(np.pi/2) * (xx-3) / 3)
                    # step_fp.footPose[1] = foot_traj[i][1] 
                    step_fp.footPose[2]+=foot_now_h
                    print("step_fp:", step_fp.footPose)
...
#     global received_nearest_point
#     if not received_nearest_point:
#         # 提取 yaw（从四元数转换）
...
#         body_poses = [
#             [msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z, deta],
#         ]
...
def nearest_point_odom_callback(msg: Odometry):
    ...
    foot_now_h=msg.twist.twist.linear.z    
    deta=yaw_deg
    to_x = msg.pose.pose.position.x
    to_y = msg.pose.pose.position.y
    to_z = msg.pose.pose.position.z
...
if __name__ == '__main__':
    ...
    rate = rospy.Rate(10)  # 10 Hz 循环频率
    rospy.loginfo("Node started and entering main loop...")

    while not rospy.is_shutdown():
        # 可在此循环中执行周期性逻辑，例如检测状态、发布数据等
        rospy.loginfo_once("Main loop running...")

        # 示例：你可以根据标志位进行周期处理，比如：
        if sign==1:
            # do something
            is_left_first_default = True # 缺省左脚先行
            # 缺省开启碰撞检测，如果默认规划的步态顺序会导致碰撞，则会自动切换到另一侧的步态,如果设置为False则不会切换步态顺序
