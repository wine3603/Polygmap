    ...
    for i in range(num):
        ...
        if i == 0:
            for xx in range(7):
                ...
                step_fp.footPose[2] = (foot_traj[i][2]+foot_h) * np.sin((math.pi - math.asin(foot_traj[i][2]/(foot_traj[i][2] +foot_h))) * xx / 6)  
                step_fp.footPose[1] = foot_traj[i][1] 
                print(np.sin((math.pi - math.asin(foot_h/(foot_traj[i][2] +foot_h)))*xx / 6))
                print("step_fp.footPose:", step_fp.footPose)
...
def get_multiple_steps_msg(body_poses, dt, is_left_first=True, collision_check=True):
    ...
    for i in range(num_steps):
        ...
        if i % 2 == 0:
            ...
            torso_traj.append((torso_pose_last + torso_pose) * 0.05)
