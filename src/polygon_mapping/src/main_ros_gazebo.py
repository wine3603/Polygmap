...
def interpolate_contour_spline(contour, num_points=10):
    """
    根据轮廓点的累积弧长均匀采点，保持曲线原形状。
    contour: Nx2 numpy数组，原始轮廓点（可以是曲线近似点）
    返回：num_points x 2 的均匀采样点
    """
    ...
    arc_lengths = np.insert(np.cumsum(seg_lengths), 0, 0)  # 起始0点

    # 创建弧长对应x,y的一维插值函数
    fx = interp1d(arc_lengths, contour[:, 0], kind='linear')
    fy = interp1d(arc_lengths, contour[:, 1], kind='linear')

    # 总长度
    total_length = arc_lengths[-1]

    # 生成均匀弧长采样点
    uniform_lengths = np.linspace(0, total_length, num_points, endpoint=False)

    # 计算采样点坐标
    x_uniform = fx(uniform_lengths)
    y_uniform = fy(uniform_lengths)

    return np.vstack((x_uniform, y_uniform)).T.astype(np.float32)
...
def main():
    ...
    try:
        ...
        marker_array_pub = rospy.Publisher('/polygon_new_markers', MarkerArray, queue_size=20)
        ...
        while True:
            ...
            if depth_image is not None and depth_image.size > 0:
                ...
                print("The depth image is empty or None. Skipping anisotropic diffusion.")
                continue              
            ...
            for i, contour in enumerate(contours):
                ...
                # 插值生成更密集轮廓点
                approx = interpolate_contour_spline(contour, num_points=200)

                # 安全检查：空或异常数据跳过
                if approx is None or len(approx) < 3:
                    continue

                # OpenCV 要求轮廓格式为 (N, 1, 2) 且 int32 类型
                approx_int = approx.astype(np.int32).reshape((-1, 1, 2))

                # 面积过滤（使用 int 类型版本）
                if cv2.contourArea(approx_int) < min_area_thresh:
                    continue

                # 可视化：绘制绿色轮廓线
                cv2.drawContours(filtered_img, [approx_int], 0, (0, 255, 0), 2)

                # 可视化：绘制红色顶点（使用 float 类型版本）
                for point in approx:
                    cv2.circle(filtered_img, tuple(point.astype(int)), 5, (0, 0, 255), -1)

                # 将轮廓顶点映射为稠密点云区域
                polygon_points = depth_to_point_cloud_region(
                    smoothed_depth_img,
                    approx,  # approx 已是 (N, 2) float32，无需 reshape
                    fx, fy, cx, cy
                )
            ...
            if len(new_polygon_list)!=0:
                last_polygon_list = new_polygon_list 
            else:
                print("new_polygon_list is not None:")
                new_polygon_list = last_polygon_list
            ...
            for i, polygon in enumerate(new_polygon_list):
                ...
                if len(polygon) > 2:
                    # Close the polygon
                    point = Point()
                    point.x, point.y, point.z = polygon[0]
                    marker.points.append(point)
