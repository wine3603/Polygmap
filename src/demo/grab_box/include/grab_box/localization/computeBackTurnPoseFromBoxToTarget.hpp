...
namespace GrabBox
{
  class ComputeBackTurnPoseFromBoxToTarget : public BT::SyncActionNode
  {
  public:
    ComputeBackTurnPoseFromBoxToTarget(const std::string& name, const BT::NodeConfiguration& config)
    ...
    static BT::PortsList providedPorts()
    {
      return {
        ...
        BT::InputPort<Eigen::Vector4d>("destination"),
    ...
    BT::NodeStatus tick() override
    {
      ...
      Eigen::Vector4d target_destination;
      ...
      // 获取 target_destination
      if (!getInput("destination", target_destination)) {
        ROS_ERROR("Failed to get input ports for destination");
        return BT::NodeStatus::FAILURE;
      }
      ...
      std::cout << "tag_pose_world_ : " << box_pose_world << std::endl;
    ...
    //根据设定的相对位置和BOX生成目标位置
      ...
      // 
      Eigen::Vector4d currentTagetDelta = target_destination - target_pose;
      target_pose(3) = 180 / M_PI * std::atan2(currentTagetDelta(1), currentTagetDelta(0));
